...
# 테스트를 위한 시간 상수 (분 단위)
INIT_TIME_MINUTES = 0.3       # 실제 3분
POST_WAIT_TIME_MINUTES = 0.3  # 요청에 따라 0.3분 (18초)로 변경
MAX_CHECK_DURATION_MINUTES = 3 # 실제 3분 (3분 동안 확인)
CHECK_INTERVAL_SECONDS = 5 # 10초에 1번씩 확인
...
def _check_visualization_subtask_success_criteria(parameter: str) -> bool:
    """visualization 서브태스크의 성공 조건을 검증합니다. (6개 PNG 파일 개수 및 Age)"""
    try:
    ...
    # 1. 파일 개수 검증
    if len(png_files) != 6:
        _log_message(parameter, "WARN", f"Visualization Validation FAILED: Expected 6 PNG files, Found: {len(png_files)} in {param_dir}.")
        return False
